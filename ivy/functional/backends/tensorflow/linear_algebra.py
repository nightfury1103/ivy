...
def tensordot(
    ...
    *,
    ...
    return tf.cast(tf.tensordot(x1, x2, axes=axes), dtype)
...
def vector_norm(
    ...
    *,
    ...
    if dtype and x.dtype != dtype:
        x = tf.cast(x, dtype)
    if ord == -float("inf"):
        return tf.reduce_min(tf.abs(x), axis, keepdims)
    elif ord == float("inf"):
        return tf.reduce_max(tf.abs(x), axis, keepdims)
    elif ord == 0:
        return tf.reduce_sum(tf.cast(x != 0, x.dtype), axis, keepdims)
    else:
        return tf.reduce_sum(tf.abs(x) ** ord, axis, keepdims) ** (1.0 / ord)
